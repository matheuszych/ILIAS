...
class ilBookingReservationsTableGUI extends ilTable2GUI
{
    ...
    public function getItems(array $filter): void
    {
        ...
        if (count($this->getSelectedUserColumns()) > 0) {
            ...
            if ($odf_ids !== []) {
                $parent_group_course = $this->access->getParentGroupCourse($this->ref_id);
                $parent_obj_id = ilObject::_lookupObjectId($parent_group_course['ref_id']);

                $privacy_settings = ilPrivacySettings::getInstance();
                $confirmation_required = $parent_group_course['type'] === 'crs'
